...
public class RePluginClassLoader extends PathClassLoader {
    ...
    public RePluginClassLoader(ClassLoader parent, ClassLoader orig,PmBase pmBase) {
...
//        }
    ...
    private void copyFieldValue(String field, ClassLoader orig) {
        try {
            ...
            if (f == null) {
                if (LOG) {
                    LogDebug.e(PLUGIN_TAG, "rpcl.cfv: null! f=" + field);
                }
                return;
            }
            ...
            if (LOG) {
                LogDebug.e(PLUGIN_TAG, "rpcl.cfv: fail! f=" + field);
            }
        }
    }
...
//            }
    ...
    protected Class<?> findClass(String className) throws ClassNotFoundException {
        ...
        if (LOG) {
            LogDebug.w(PLUGIN_TAG, "NRH lcl.fc: c=" + className);
        }
    ...
    protected Package getPackage(String name) {
        ...
        if (name != null && !name.isEmpty()) {
            ...
            if (pack == null) {
                if (LOG) {
                    LogDebug.w(PLUGIN_TAG, "NRH lcl.gp.1: n=" + name);
                }
                pack = super.getPackage(name);
            }
            if (pack == null) {
                if (LOG) {
                    LogDebug.w(PLUGIN_TAG, "NRH lcl.gp.2: n=" + name);
                }
